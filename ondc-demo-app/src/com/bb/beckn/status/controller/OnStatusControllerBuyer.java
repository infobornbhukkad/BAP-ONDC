...
@RestController
public class OnStatusControllerBuyer
{
    ...
    public ResponseEntity<String> onStatus(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
        OnStatusControllerBuyer.log.info("The body in {} adaptor is {}", (Object)"status", (Object)this.jsonUtil.unpretty(body));
        try {
            final OnSchema model = (OnSchema)this.jsonUtil.toModel(body, (Class)OnSchema.class);
            OnStatusControllerBuyer.log.info("data in buyer controller adaptor [which is will now send back to buyer internal api] is {}", (Object)model.toString());
            ...
            OnStatusControllerBuyer.log.info("does seller {} requires to be authenticated ? {}", (Object)bppId, (Object)authenticate);
            if (authenticate) {
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnStatusControllerBuyer.class);
    }
}
