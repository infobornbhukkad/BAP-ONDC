...
@RestController
public class SelectControllerSeller
{
    private static final Logger log;
    @Autowired
    private SelectServiceSeller service;
    @Autowired
    ...
    @PostMapping({ "/seller/adaptor/select" })
    public ResponseEntity<String> select(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders, final HttpServletRequest servletRequest) throws JsonProcessingException {
        SelectControllerSeller.log.info("The body in {} adaptor is {}", (Object)"select", (Object)this.jsonUtil.unpretty(body));
        SelectControllerSeller.log.info("Entity type is {}", (Object)this.entityType);
        ...
        final ConfigModel configModel = this.configService.loadApplicationConfiguration(context.getBppId(), "select");
        final String bapId = context.getBapId();
        final boolean authenticate = configModel.getMatchedApi().isHeaderAuthentication();
        SelectControllerSeller.log.info("does buyer {} requires to be authenticated ? {}", (Object)bapId, (Object)authenticate);
        if (authenticate) {
        ...
        return (ResponseEntity<String>)this.service.select(httpHeaders, model);
    }
    ...
    static {
        log = LoggerFactory.getLogger((Class)SelectControllerSeller.class);
    }
}
